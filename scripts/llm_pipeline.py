...
def generate_all_texts(config: EvaluationConfig, start_point: int) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for model_idx, (model_path, model_type) in enumerate(zip(config.model_paths, config.model_types)):
        ...
        # Load model once per model_path instead of once per parameter combination
        if model_type == 'bayesian':
            loaded_model, loaded_samples, loaded_encode, loaded_decode = load_bayesian_model(
                model_path, device=config.device
            )
            ...
            for temperature in config.temperatures:
                for top_k in config.top_k_values:
                    for num_samples in config.num_samples_values:
                        generation_count += 1
                        if generation_count <= start_point:
                          continue
                        # Generate unique ID
                        ...
                        try:
                            if model_type == 'bayesian':
                                print(f"Generating sample {generation_count}")
                                # Generate text with pre-loaded Bayesian model
                                generated_text, unc_info = generate_text_bayesian_from_loaded(
                                    model=loaded_model,
                                    collected_samples=loaded_samples,
                                    encode=loaded_encode,
                        ...
                        if generation_count % 10 == 0:
                            checkpoint_path = f'/content/drive/MyDrive/samplers_models/prompt_generation0503/pretrained-prior/checkpoint_gen_{generation_count}.json'
                            with open(checkpoint_path, 'w') as f:
                                json.dump(results, f, indent=2)
                            print(f"Saved checkpoint at generation {generation_count}")
...
def _evaluate_with_local_qwen(results_by_prompt: Dict[str, Dict],
    ...
    Args:
        ...
        batch_size: Number of texts per Qwen call (keep small to avoid OOM from huge KV cache)
    ...
    try:
        ...
        # Free any lingering GPU memory from generation phase before loading judge model
        gc.collect()
        ...
        for prompt_idx, (prompt, results) in enumerate(results_by_prompt.items(), 1):
            ...
            # Split texts into smaller batches to avoid OOM from huge KV cache
            items = list(results.items())
            for batch_start in range(0, len(items), batch_size):
                batch = dict(items[batch_start:batch_start + batch_size])
                batch_label = f"{batch_start + 1}-{min(batch_start + batch_size, len(items))}/{len(items)}"
                print(f"  Batch {batch_label}...", end='', flush=True)
...
def run_evaluation_pipeline(
    ...
    generation_results = generate_all_texts(config, start_point=start_point)
